...
`demo-memory`, `demo-algorithms`, and `sw-mlpl`.
...
- Name files explicitly when staging and commit `.agentrail/` metadata with the feature it records.
- Use a focused failing fixture or assertion before or with executable changes.
- Prefer pure free functions, composition, top-down delegation, and define-once/invoke-many helpers.
- Separate decisions from effects: `.mlpl` computes values; thin shell runners perform effects.
- Do not modify `../sw-mlpl` here. Minimize needs in `docs/upstream-contract.md` for separate upstream work.
- Honor `$MLPL`; never install or overwrite a user's sw-MLPL binary.
- After `agentrail complete`, stop and name the next highest-value unblocked step.

## Task runner policy

- Use `just` and the root `justfile` for routine build, test, and audit tasks.
- Do not add a `Makefile` or invoke `make`.
- Keep recipes thin and delegate validation and execution to `scripts/`.

## Demo quality rules

- Keep demos deterministic, standalone, and self-checking.
- End executable demos with `ok(...)` on success and `err(...)` on failure.
- Exercise empty, singleton, boundary, duplicate, and invalid cases where applicable.
- Document invariants, logical and copy complexity, and every remaining explicit loop.
- Do not claim laziness, streaming, fusion, structural sharing, or physical performance without evidence.
- Add every executable demo to the catalog and `scripts/check` in the same feature step.
